...
class ApiError with _$ApiError implements Exception {
  ...
  String get message {
    return maybeWhen(
      orElse: () => S.current.common_error_unexpected,
  ...
  String get title => maybeWhen(
        (code, message) => S.current.common_error,
        unauthorized: (message) => S.current.common_error_unauthorized,
        network: (code, __) => code == HttpStatus.internalServerError
            ? S.current.common_error_internal_server
            : S.current.common_error,
        orElse: () => S.current.common_error,
      );
}
